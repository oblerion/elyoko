...
struct LightManager lightmanager = {0};
...
void LM_AddPointLight(Vector3 position, Vector3 target, Color color)
{
    ...
    {
        lightmanager.lights[lightmanager.count] = CreateLight(LIGHT_POINT,position,target,color,lightmanager.lightshader,lightmanager.count);
        lightmanager.count++;
    }
}
...
void LM_AddDirectionalLight(Vector3 position, Vector3 target, Color color)
{
    if(lightmanager.count<MAX_LIGHTS)
    {
        lightmanager.lights[lightmanager.count] = CreateLight(LIGHT_DIRECTIONAL,position,target,color,lightmanager.lightshader,lightmanager.count);
        lightmanager.count++;
    }
}
